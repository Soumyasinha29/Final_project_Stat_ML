log_model <- function(filtered_dataset) {
    ...
    # Plot of the relationship between gotAspirin24 and the response variable
    plot(ggplot(filtered_dataset, aes(x = gotAspirin24, fill = isDead)) +
    geom_bar(position = "fill") +
    xlab("Got Aspirin 24") +
    ylab("Proportion") +
    ggtitle("Relationship Between Got Aspirin 24 and Survival Status"))

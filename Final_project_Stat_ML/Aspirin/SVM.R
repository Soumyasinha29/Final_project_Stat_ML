base_svm_model <- function(filtered_dataset) {
    ...
    # To balance the model uncomment the below section
    ...
    f1 <- formula(isDead ~ gender + ethnicity + race + gotAspirin24 + gotPE + gotDVT)
    ...
    print(train_svmrbk$results[which.max(train_svmrbk$results$Accuracy),])
    ...
    confusion_mtx <- confusionMatrix(table(pred_test, test_data$isDead))
    print(confusion_mtx$overall['Accuracy'])

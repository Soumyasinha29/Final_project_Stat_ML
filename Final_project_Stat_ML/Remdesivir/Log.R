log_model <- function(filtered_dataset) {
    ...
    # Plot of the relationship between gotRemdesivir120 and the response variable
    plot(ggplot(filtered_dataset, aes(x = gotRemdesivir120, fill = isDead)) +
    geom_bar(position = "fill") +
    xlab("Got Remdesivir 120") +
    ylab("Proportion") +
    ggtitle("Relationship Between Got Remdesivir 120 and Survival Status"))

log_model <- function(filtered_dataset) {
    ...
    filtered_dataset$gotHeparin24 <- factor(filtered_dataset$gotHeparin24)
    ...
    # Plot of the relationship between gotHeparin24 and the response variable
    plot(ggplot(filtered_dataset, aes(x = gotHeparin24, fill = isDead)) +
    geom_bar(position = "fill") +
    xlab("Got Heparin 24") +
    ylab("Proportion") +
    ggtitle("Relationship Between Got Heparin 24 and Survival Status"))
